...
async function postSync(payload) {
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      console.warn("⚠️ فشل الإرسال", res.status, data);
      if (res.status < 500) return { ok: false, data };
    } catch (e) {
      console.warn(`⚠️ شبكة (محاولة ${attempt}/3):`, e.message);
    }
...
function writeExcel() {
  ...
  const file = path.join(
    outDir,
    `najiz-data-puppeteer-${new Date().toISOString().replace(/[:.]/g, "-")}.xlsx`,
  );
...
async function injectScraper(page) {
  await page.evaluate(CHROME_STUB).catch(() => {});
...
async function safeGoto(page, url) {
  try {
    ...
    console.warn("⚠️ safeGoto failed:", e.message);
  }
...
async function clickRowByIdentifier(page, identifier) {
  ...
  if (clicked) await page.waitForTimeout(FIXTURE_BASE ? 500 : 3000);
  return clicked;
}
...
async function clickRowByIdentifierWithFallback(page, identifier) {
  ...
  const handles = await page.$x(`//*[contains(normalize-space(string(.)), "${escaped}")]`);
  for (const handle of handles) {
    try {
      const inner = await handle.$('a, button, [role="button"], [role="link"]');
      if (inner) {
...
(async () => {
  console.log("⚖️  بوت ناجز — منصة العدالة (Puppeteer)");
  ...
  try {
    ...
    for (const step of STEPS) {
      ...
      if (n) {
        ...
        const res = await postSync(payload);
        console.log(
          res.ok ? `  📤 قائمة ${step.label}: ${n} عنصر ✓` : `  ⚠️ فشل إرسال قائمة ${step.label}`,
        );
