...
-- 2) plpgsql utility functions (no table refs at parse time)
...
-- 3) TABLES + grants + RLS + owner policies + triggers (no dependencies on SQL helpers)
...
CREATE UNIQUE INDEX IF NOT EXISTS uniq_clients_owner_email ON public.clients(owner_id,lower(email)) WHERE email IS NOT NULL;
...
CREATE TABLE IF NOT EXISTS public.employees (
  ...
  portal_access_code TEXT, portal_username TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE INDEX IF NOT EXISTS idx_cases_assigned_employee ON public.cases(assigned_employee_id);
...
CREATE POLICY "employee read assigned tasks" ON public.tasks FOR SELECT TO authenticated USING (EXISTS(SELECT 1 FROM public.employees WHERE employees.id=tasks.employee_id AND employees.user_id=auth.uid() AND employees.owner_id=tasks.owner_id));
DROP POLICY IF EXISTS "employee updates assigned tasks" ON public.tasks;
CREATE POLICY "employee updates assigned tasks" ON public.tasks FOR UPDATE TO authenticated USING (EXISTS(SELECT 1 FROM public.employees WHERE employees.id=tasks.employee_id AND employees.user_id=auth.uid() AND employees.owner_id=tasks.owner_id)) WITH CHECK (EXISTS(SELECT 1 FROM public.employees WHERE employees.id=tasks.employee_id AND employees.user_id=auth.uid() AND employees.owner_id=tasks.owner_id));
...
CREATE POLICY "client manage own portal msgs" ON public.portal_messages FOR ALL TO authenticated
  ...
  WITH CHECK (client_id IN (SELECT id FROM public.clients WHERE portal_user_id=auth.uid()) AND sender_id=auth.uid() AND sender_role='client');
...
CREATE POLICY "sender inserts own message" ON public.employee_messages FOR INSERT TO authenticated WITH CHECK (auth.uid()=sender_id AND (auth.uid()=owner_id OR EXISTS(SELECT 1 FROM public.employees e WHERE e.user_id=auth.uid() AND e.owner_id=employee_messages.owner_id)) AND (recipient_id=owner_id OR EXISTS(SELECT 1 FROM public.employees e WHERE e.user_id=recipient_id AND e.owner_id=employee_messages.owner_id)));
...
-- 4) SQL helper functions (now safe — tables exist)
...
CREATE OR REPLACE FUNCTION public.employee_can_access_case(_case_id UUID, _user_id UUID) RETURNS BOOLEAN LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public AS $$
  SELECT EXISTS(SELECT 1 FROM public.employees e JOIN public.cases c ON c.id=_case_id WHERE e.user_id=_user_id AND e.is_active=true AND e.owner_id=c.owner_id AND (c.assigned_employee_id=e.id OR c.id=ANY(COALESCE(e.assigned_cases,ARRAY[]::uuid[])) OR c.client_id=ANY(COALESCE(e.assigned_clients,ARRAY[]::uuid[]))));
$$;

CREATE OR REPLACE FUNCTION public.employee_can_access_client(_client_id UUID, _user_id UUID) RETURNS BOOLEAN LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public AS $$
  SELECT EXISTS(SELECT 1 FROM public.employees e JOIN public.clients cl ON cl.id=_client_id WHERE e.user_id=_user_id AND e.is_active=true AND e.owner_id=cl.owner_id AND (cl.id=ANY(COALESCE(e.assigned_clients,ARRAY[]::uuid[])) OR EXISTS(SELECT 1 FROM public.cases ca WHERE ca.client_id=cl.id AND ca.id=ANY(COALESCE(e.assigned_cases,ARRAY[]::uuid[])))));
$$;

CREATE OR REPLACE FUNCTION public.has_doc_permission(_case_id uuid, _user_id uuid, _perm public.doc_permission) RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public AS $$
  SELECT EXISTS(SELECT 1 FROM public.cases c WHERE c.id=_case_id AND c.owner_id=_user_id) OR EXISTS(SELECT 1 FROM public.document_permissions dp WHERE dp.case_id=_case_id AND dp.user_id=_user_id AND (dp.permission=_perm OR dp.permission='manage'));
$$;

CREATE OR REPLACE FUNCTION public.has_doc_permission(_case_id UUID, _perm public.doc_permission, _user_id UUID) RETURNS BOOLEAN LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public AS $$
  SELECT EXISTS(SELECT 1 FROM public.document_permissions dp WHERE dp.user_id=_user_id AND (dp.case_id IS NULL OR dp.case_id=_case_id) AND (dp.permission=_perm OR dp.permission='manage'));
$$;

CREATE OR REPLACE FUNCTION public.get_employees_directory() RETURNS TABLE(id uuid,user_id uuid,owner_id uuid,full_name text,job_title text,is_active boolean) LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public AS $$
  SELECT e.id,e.user_id,e.owner_id,e.full_name,e.job_title,e.is_active FROM public.employees e
  WHERE auth.uid() IS NOT NULL AND (e.owner_id=auth.uid() OR e.user_id=auth.uid() OR EXISTS(SELECT 1 FROM public.employees me WHERE me.user_id=auth.uid() AND me.is_active=true AND me.owner_id=e.owner_id));
$$;
...
-- 5) plpgsql functions depending on tables
CREATE OR REPLACE FUNCTION public.handle_new_user() RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
BEGIN
  INSERT INTO public.profiles(id,full_name,email) VALUES(NEW.id,COALESCE(NEW.raw_user_meta_data->>'full_name',NEW.email),NEW.email) ON CONFLICT(id) DO UPDATE SET email=EXCLUDED.email,full_name=COALESCE(public.profiles.full_name,EXCLUDED.full_name);
  INSERT INTO public.user_roles(user_id,role) VALUES(NEW.id,'lawyer') ON CONFLICT(user_id,role) DO NOTHING;
  RETURN NEW;
END; $$;

CREATE OR REPLACE FUNCTION public.link_current_user_to_portal(_access_code TEXT DEFAULT NULL, _account_type TEXT DEFAULT 'client') RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
...
BEGIN
  ...
  IF _access_code IS NULL OR length(btrim(_access_code))=0 THEN RETURN jsonb_build_object('ok',false,'error','missing_code'); END IF;
  IF _account_type='employee' THEN UPDATE public.employees SET user_id=_uid WHERE portal_access_code=_access_code AND user_id IS NULL;
  ELSE UPDATE public.clients SET portal_user_id=_uid WHERE portal_access_code=_access_code AND portal_user_id IS NULL; END IF;
...
-- 7) Policies depending on SQL helper functions
...
-- 9) Storage policies (bucket `case-documents` is created separately via storage tool)
DROP POLICY IF EXISTS "case_documents_select_own" ON storage.objects;
CREATE POLICY "case_documents_select_own" ON storage.objects FOR SELECT TO authenticated USING (bucket_id='case-documents' AND auth.uid()::text=(storage.foldername(name))[1]);
DROP POLICY IF EXISTS "case_documents_insert_own" ON storage.objects;
CREATE POLICY "case_documents_insert_own" ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id='case-documents' AND auth.uid()::text=(storage.foldername(name))[1]);
DROP POLICY IF EXISTS "case_documents_update_own" ON storage.objects;
CREATE POLICY "case_documents_update_own" ON storage.objects FOR UPDATE TO authenticated USING (bucket_id='case-documents' AND auth.uid()::text=(storage.foldername(name))[1]) WITH CHECK (bucket_id='case-documents' AND auth.uid()::text=(storage.foldername(name))[1]);
DROP POLICY IF EXISTS "case_documents_delete_own" ON storage.objects;
CREATE POLICY "case_documents_delete_own" ON storage.objects FOR DELETE TO authenticated USING (bucket_id='case-documents' AND auth.uid()::text=(storage.foldername(name))[1]);